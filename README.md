...
### iOS (Safari) - 중요!
1. **HTTPS 필수**: iOS는 HTTPS에서만 PWA 설치 가능
2. Safari에서 앱을 열어주세요
3. 하단의 **공유 버튼 (📤)** → **"홈 화면에 추가"** 선택
4. 앱 이름을 확인하고 **"추가"** 버튼 클릭
5. 홈 화면에서 앱 아이콘 확인
...
### 설치 버튼이 보이지 않는 경우 (iOS)
- **HTTPS 연결 필수** - iOS Safari는 HTTP에서 PWA 설치 불가
- 자체 서명 인증서 경고 시 "고급" → "계속 진행" 선택
- Safari 브라우저에서만 설치 가능 (Chrome 앱 불가)
- 앱이 3초 후 자동으로 설치 안내 표시
